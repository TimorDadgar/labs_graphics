...
void normCalculator(float* p, unsigned short* f,float* u_v, glm::vec4* n, int len_f, glm::vec4* t, glm::vec4* b) {
	int i, j;
	glm::vec2 u1, u2, u3;
	glm::vec3 p1, p2, p3;
	...
	for (i = 0; i < len_f / 3; i++)
	{
		...
		u1 = glm::vec2(u_v[p1_start * 2], u_v[p1_start * 2 + 1]);
		u2 = glm::vec2(u_v[p2_start * 2], u_v[p2_start * 2 + 1]);
		u3 = glm::vec2(u_v[p3_start * 2], u_v[p3_start * 2 + 1]);
		...
		float r = f1[0] * f2[1] - f1[1] * f2[0];
		glm::vec3 t_temp = (f2[1] * x - f1[1] * y)/r;
		glm::vec3 b_temp = (f1[0] * y - f2[0] * x) / r;

		n[p1_start] = glm::vec4(n_temp, 0);
		n[p2_start] = glm::vec4(n_temp, 0);
		n[p3_start] = glm::vec4(n_temp, 0);

		t[p1_start] = glm::vec4(n_temp, 0);
		t[p2_start] = glm::vec4(n_temp, 0);
		t[p3_start] = glm::vec4(n_temp, 0);

		b[p1_start] = glm::vec4(n_temp, 0);
		b[p2_start] = glm::vec4(n_temp, 0);
		b[p3_start] = glm::vec4(n_temp, 0);
...
int main(int argc, char const* argv[])
{
	...
	glm::vec4 t_vec4[60];
	glm::vec4 b_vec4[60];
	normCalculator(points, faces, uv_coords, norm_vec4, sizeof(faces) / sizeof(faces[0]), t_vec4, b_vec4); //sizeof is divided by 2 since unsigned in is 2 bytes
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(t_vec4), &t_vec4, GL_STATIC_DRAW); //Chose GL_STATIC_DRAW because it is accessing data once and then only used for drawing
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(b_vec4), &b_vec4, GL_STATIC_DRAW); //Chose GL_STATIC_DRAW because it is accessing data once and then only used for drawing
	...
	glUniform1i(glGetUniformLocation(shader_program, "normal_sampler"), 1);
	glBindTexture(GL_TEXTURE_2D, normal_handle);



	//loading textures and creating mipmap

	/*Mipmap for numbers and texture_handle*/
	...
	/*Mipmap for cubemap and environment using environment_handle*/
	...
	/*Mipmap for bumpmapand normal_handle*/
