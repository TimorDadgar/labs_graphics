...
void main () {
	...
	vec4 env = texture(env_sampler, reflection_vector);

	float r = 0.8;

	// Haven't done scaling on the normal_sampler
	mat4 TBN = mat4(tangent, bitangent, vec4(normal_n,0), vec4(0,0,0,1));

	vec4 frag_vec4;

	for ( int l = 0; l < light_count; ++l )
	{
		frag_vec4 = vec4(light_colour[0]*3 * cook_torrance(light_direction, camera_direction) * max(0.2f, (dot(normalize(light_direction), normalize(normal)))), 0);
	}

	frag_colour = vec3( TBN * frag_vec4 );

	frag_colour = r*vec3(env)+ (1-r)*frag_colour;
}
